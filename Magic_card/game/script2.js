...
function create_card(x, y, cardData,is_player) {
    ...
    function createIcon(role, x) {
        ...
        let icon = scene.add.image(x, Height / 2, role).setDisplaySize(20, 20)
        ...
        background.on('pointerdown', () => {
            ...
            alert(container.getData('cardData'));
        ...
        // const container  = scene.add.container(0,0).setSize(7.5, 7.5);
        // container.add()
    ...
    container.add([cardImage, cardBorder, attack, effect, defense ]);
    ...
    container.on('pointerdown', () => {
        if(is_player === 1){
            containerP.handContainer.forEach(Item => {
                ...
                database[Item.getData('cardData')].handslect.forEach(index => Item.list[index].visible = (Item === container))
            });
            containerP.fieldContainer.forEach(Item =>{
                Item.list[1].visible = (Item === container);  // 邊框
                database[Item.getData('cardData')].fieldslect.forEach(index => Item.list[index].visible = (Item === container))
            });
        }
    });
    return container;
